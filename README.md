...
1. Create an E-Document from a Business Central document that can be sent to a designated endpoint: Check, Create, CreateBatch
2. Receive a document from the endpoint: GetBasicInfoFromReceivedDocument, GetCompleteInfoFromReceivedDocument
...
- GetBasicInfoFromReceivedDocument: use it to get the basic information of an E-Document from received blob.
...
- GetCompleteInfoFromReceivedDocument: Use it to create a document from imported blob.
...
Note: Do not use validation when assigning the field to header and lines to purchase document, instead use these 2 events OnBeforeProcessHeaderFieldsAssignment, OnBeforeProcessHeaderFieldsValidation in codeunit "E-Document Create Purch. Doc." to mark which fields should be assigned and validated respectively.
